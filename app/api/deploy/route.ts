...
function readErrorMessage(payload: unknown): string {
  ...
  return "Backend deploy request failed.";
}
...
export async function POST(request: Request) {
  ...
  const terraformCode =
    payload &&
    typeof payload === "object" &&
    "terraform_code" in payload &&
    typeof payload.terraform_code === "string"
  ...
  if (!terraformCode) {
    return NextResponse.json(
      { message: "Terraform code is required for GitOps delivery." },
      { status: 400 },
    );
  }

  try {
    const backendResponse = await fetch(`${backendApiBaseUrl}/deploy`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ prompt, terraform_code: terraformCode }),
      cache: "no-store",
    });
